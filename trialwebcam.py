...
NUMBER_DESCRIPTIONS = {
    ...
    '2': "Index and middle fingers extend up (like a 'V' but backwards); others folded.",
    ...
    '6': "Index thouches the lower partb of thumb; index, other fingers bend.",
    '7': "index and thumb make letter L inverted; other fingers bend.",
    '8': "Middle finger and thumb touch the middle of index; other fingers bend.",
    '9': "Index thouches the lower partb of thumb; (6 inverted)index, other fingers bend",
    '10': "Thumbs up for both hands side to side (or show '1' then '0').",
}
...
def _find_working_camera(timeout_sec=3):
    """
    Try camera indices 0-4 and return the first one that actually
    delivers a frame within timeout_sec. Returns (cap, index) or (None, -1).
    """
    import threading
    ...
    for idx in range(5):
        ...
        for backend in [cv2.CAP_V4L2, cv2.CAP_ANY]:
            ...
            # Set a tight read timeout via a thread
            frame_holder = [None]
            def _read():
                ret, f = cap.read()
            ...
            if frame_holder[0] is not None:
                print(f"  ✅ Camera found at index {idx} (backend {backend})")
                return cap, idx
...
def run_webcam_practice(target_sign):
    ...
    Falls back to an interactive demo mode if no camera is available.
    """
    desc = SIGN_DESCRIPTIONS.get(target_sign) or NUMBER_DESCRIPTIONS.get(target_sign, "")
    print(f"\n  📸 Setting up practice for sign: [{target_sign}]")
    print(f"  Tip: {desc}\n")

    print("  Searching for a working camera (indices 0-4) …")
    cap, cam_idx = _find_working_camera(timeout_sec=3)

    if cap is None:
        print("  ⚠  No camera responded in time.")
        print("  Possible causes:")
        print("    • Camera is in use by another app  (close it and try again)")
        print("    • Permissions: run  sudo chmod 777 /dev/video*  then retry")
        print("    • Virtual machine / container with no camera passthrough")
        print()
        _interactive_demo(target_sign)
        return

    score_history = []
    best_score    = 0
    consecutive_no_hand = 0
    print("  Camera open! Show your hand and press [SPACE] to score. [Q] quits.\n")

    while True:
        ret, frame = cap.read()
        if not ret or frame is None:
            consecutive_no_hand += 1
            if consecutive_no_hand > 30:
                print("  ⚠  Camera stopped delivering frames – exiting webcam mode.")
                break
            time.sleep(0.03)
            continue
        consecutive_no_hand = 0
        ...
        cv2.putText(annotated,
                    f"Sign: [{target_sign}]   SPACE = Capture   Q = Quit",
                    (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (255, 255, 255), 2)

        hand_txt   = "Hand detected ✓" if hand_found else "No hand – show palm to camera"
        hand_color = (0, 230, 80)      if hand_found else (0, 80, 230)
        cv2.putText(annotated, hand_txt, (10, 58),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.65, hand_color, 2)

        if score_history:
            last = score_history[-1]
            bar_color = (0, 200, 0) if last >= 60 else (0, 100, 220)
            cv2.putText(annotated, f"Last: {last}%  Best: {best_score}%",
                        (w - 260, 58), cv2.FONT_HERSHEY_SIMPLEX, 0.62, bar_color, 2)

        # ── Finger guide (bottom strip) ───────────────────────
        strip_y = h - 35
        cv2.rectangle(annotated, (0, strip_y - 5), (w, h), (20, 20, 20), -1)
        short_desc = desc[:90] + ("…" if len(desc) > 90 else "")
        cv2.putText(annotated, short_desc, (8, h - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.45, (200, 230, 255), 1)

        cv2.imshow("Uganda Sign Language Instructor  [Cam {}]".format(cam_idx), annotated)
        key = cv2.waitKey(1) & 0xFF

        if key == ord(' '):
            if not hand_found:
                print("  ⚠  No hand in frame – move your hand closer to the camera.")
                continue
            ...
            if landmarks is not None:
                ...
                msg = feedback_message(score)
                print(f"\n  📊  Score: {score}%  —  {msg}")

                # Flash result on screen
                border_color = (0, 210, 0) if score >= 60 else (0, 30, 210)
                flash = annotated.copy()
                cv2.rectangle(flash, (0, 0), (w, h), border_color, 10)
                score_txt = f"{score}%"
                txt_size  = cv2.getTextSize(score_txt, cv2.FONT_HERSHEY_SIMPLEX, 3, 5)[0]
                tx = (w - txt_size[0]) // 2
                ty = (h + txt_size[1]) // 2
                cv2.putText(flash, score_txt, (tx, ty),
                            cv2.FONT_HERSHEY_SIMPLEX, 3, border_color, 5)
                cv2.putText(flash, msg.replace("🌟","").replace("👍","").replace("😊","")
                                       .replace("📖","").replace("❌","").strip(),
                            (10, ty + 55), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 255), 2)
                cv2.imshow("Uganda Sign Language Instructor  [Cam {}]".format(cam_idx), flash)
                cv2.waitKey(1800)
            else:
                print("  ⚠  Hand visible but landmarks unreadable – adjust lighting or distance.")
    ...
    # ── Session summary ───────────────────────────────────────
    if score_history:
        avg = int(np.mean(score_history))
        print(f"\n  {'─'*45}")
        print(f"  Session Summary  –  Sign [{target_sign}]")
        print(f"  {'─'*45}")
        print(f"  Attempts  : {len(score_history)}")
        print(f"  Best Score: {best_score}%  {feedback_message(best_score)}")
        print(f"  Average   : {avg}%")
        # ASCII score bar
        bar = "█" * (best_score // 5) + "░" * (20 - best_score // 5)
        print(f"  Progress  : [{bar}] {best_score}%")
        print(f"  {'─'*45}")
        show_youtube_resources(best_score)
    else:
        show_youtube_resources(0)


def _interactive_demo(target_sign):
    """
    Interactive demo mode – no webcam needed.
    Simulates multiple 'attempts' with realistic varied scores so the
    user can see how the scoring and feedback loop works.
    """
    print("  ┌─────────────────────────────────────────────────────┐")
    print(f"  │  DEMO MODE  –  Practicing sign: [{target_sign}]             ")
    print("  │  Simulating hand landmark captures from your camera. │")
    print("  └─────────────────────────────────────────────────────┘\n")
    print("  In this demo each 'attempt' simulates pressing SPACE on the webcam.")
    print("  When your camera is connected you will use the LIVE window instead.\n")

    desc = SIGN_DESCRIPTIONS.get(target_sign) or NUMBER_DESCRIPTIONS.get(target_sign, "")
    print(f"  How to sign [{target_sign}]: {desc}\n")

    attempts = int(input("  How many practice attempts to simulate? (1-10, default 5): ").strip() or "5")
    attempts = max(1, min(10, attempts))

    score_history = []
    # Noise decreases each attempt – simulates the user improving
    for i in range(1, attempts + 1):
        noise_level = max(0.05, 0.30 - i * 0.025)   # gets better each round
        fake_lm = generate_reference_landmarks(target_sign, seed_offset=i*7)
        fake_lm += np.random.randn(63) * noise_level
        score = compute_score(fake_lm, target_sign)
        score_history.append(score)

        bar   = "█" * (score // 5) + "░" * (20 - score // 5)
        print(f"  Attempt {i:2d}: [{bar}] {score:3d}%  —  {feedback_message(score)}")
        time.sleep(0.4)

    best  = max(score_history)
    avg   = int(np.mean(score_history))
    trend = " Improving!" if score_history[-1] >= score_history[0] else "📉 Keep going!"

    print(f"\n  {'─'*50}")
    print(f"  Demo Summary  –  Sign [{target_sign}]")
    print(f"  {'─'*50}")
    print(f"  Attempts  : {len(score_history)}")
    print(f"  Best Score: {best}%  {feedback_message(best)}")
    print(f"  Average   : {avg}%")
    print(f"  Trend     : {trend}")
    bar = "█" * (best // 5) + "░" * (20 - best // 5)
    print(f"  Progress  : [{bar}] {best}%")
    print(f"  {'─'*50}")
    print("\n  Tip: Connect your webcam for LIVE scoring with real hand detection!\n")

    show_youtube_resources(best)
